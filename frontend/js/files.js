  ...
  async function render(container, searchQuery = "") {
    ...
    try {
      ...
      if (total === 0) {
        container.innerHTML = `
          <div class="flex flex-col items-center justify-center py-16 text-center animate-[fadeIn_0.3s_ease]">
            ...
            <p class="text-sm text-muted-foreground mt-2">${searchQuery ? I18n.t("files.search_hint") : I18n.t("files.get_started")}</p>
          </div>`;
        return;
      }
          ...
          <table class="w-full">
            <thead>
              <tr class="bg-muted border-b border-border">
                ...
                <th class="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">${I18n.t("common.size")}</th>
                <th class="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider hidden sm:table-cell">${I18n.t("common.type")}</th>
      ...
      folders.forEach((folder) => {
        ...
        html += `
          <tr class="border-b border-border last:border-0 hover:bg-muted transition-colors duration-150 file-list-enter" style="background: color-mix(in srgb, var(--primary) 3%, transparent);">
            <td class="px-4 py-3">
              <div class="flex items-center gap-2.5 cursor-pointer" onclick="DirectoryUI.navigate(${folder.id})">
                ...
                <div class="min-w-0">
                  ...
                  <div class="text-xs text-muted-foreground">${I18n.t("files.folder")}</div>
                </div>
              </div>
            </td>
            <td class="px-4 py-3 text-sm text-muted-foreground whitespace-nowrap">—</td>
            ...
            <td class="px-4 py-3 text-sm text-muted-foreground whitespace-nowrap hidden md:table-cell">${formatDate(folder.created_at)}</td>
            <td class="px-4 py-3 text-right">
              <div class="flex items-center justify-end gap-1">
                <button class="p-2 rounded-md text-muted-foreground hover:text-primary hover:bg-muted transition-all cursor-pointer bg-transparent border-none" onclick="DirectoryUI.navigate(${folder.id})" title="${I18n.t("files.tooltip_open")}">
                ...
                <button class="p-2 rounded-md text-muted-foreground hover:text-primary hover:bg-muted transition-all cursor-pointer bg-transparent border-none" onclick="DirectoryUI.openMoveModal(${folder.id}, '${safeName}')" title="${I18n.t("files.tooltip_move")}">
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="5 9 2 12 5 15"/><polyline points="9 5 12 2 15 5"/><polyline points="15 19 12 22 9 19"/><polyline points="19 9 22 12 19 15"/><line x1="2" y1="12" x2="22" y2="12"/><line x1="12" y1="2" x2="12" y2="22"/></svg>
                </button>
                <button class="p-2 rounded-md text-muted-foreground hover:text-error hover:bg-[rgba(196,69,69,0.08)] transition-all cursor-pointer bg-transparent border-none" onclick="DirectoryUI.deleteFolder(${folder.id}, '${safeName}')" title="${I18n.t("files.tooltip_delete")}">
      ...
      files.forEach((file) => {
        ...
        html += `
          <tr class="border-b border-border last:border-0 hover:bg-muted transition-colors duration-150 file-list-enter">
              <td class="px-4 py-3">
            ...
            <td class="px-4 py-3 text-sm text-muted-foreground whitespace-nowrap">${file.file_size_formatted || "—"}</td>
            ...
            <td class="px-4 py-3 text-sm text-muted-foreground whitespace-nowrap hidden md:table-cell">${formatDate(file.created_at)}</td>
            <td class="px-4 py-3 text-right">
              <div class="flex items-center justify-end gap-1">
                <button class="p-2 rounded-md text-muted-foreground hover:text-primary hover:bg-muted transition-all cursor-pointer bg-transparent border-none" onclick="Download.handle(${file.id}, '${safeName}')" title="${I18n.t("files.tooltip_download")}">
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
                </button>
                <button class="p-2 rounded-md text-muted-foreground hover:text-primary hover:bg-muted transition-all cursor-pointer bg-transparent border-none" onclick="UI.openShareModal(${file.id}, '${safeName}')" title="${I18n.t("files.tooltip_share")}">
                ...
                <button class="p-2 rounded-md text-muted-foreground hover:text-primary hover:bg-muted transition-all cursor-pointer bg-transparent border-none" onclick="PublicLinkUI.openModal(${file.id}, '${safeName}')" title="${I18n.t("files.tooltip_public_link")}">
                ...
                <button class="p-2 rounded-md text-muted-foreground hover:text-primary hover:bg-muted transition-all cursor-pointer bg-transparent border-none" onclick="FileList.verifyIntegrity(${file.id})" title="${I18n.t("files.tooltip_verify")}">
                ...
                <button class="p-2 rounded-md text-muted-foreground hover:text-primary hover:bg-muted transition-all cursor-pointer bg-transparent border-none" onclick="DirectoryUI.openMoveModal(${file.id}, '${safeName}')" title="${I18n.t("files.tooltip_move")}">
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="5 9 2 12 5 15"/><polyline points="9 5 12 2 15 5"/><polyline points="15 19 12 22 9 19"/><polyline points="19 9 22 12 19 15"/><line x1="2" y1="12" x2="22" y2="12"/><line x1="12" y1="2" x2="12" y2="22"/></svg>
                </button>
                ${(() => {
                  const user = App.getCurrentUser();
                  if (user?.role !== "admin") return "";
                  return `<button class="p-2 rounded-md text-muted-foreground hover:text-primary hover:bg-muted transition-all cursor-pointer bg-transparent border-none" onclick="SecurityUI.renderFileAnalysis(${file.id})" title="${I18n.t("files.tooltip_analyze")}">
                ...
                <button class="p-2 rounded-md text-muted-foreground hover:text-error hover:bg-[rgba(196,69,69,0.08)] transition-all cursor-pointer bg-transparent border-none" onclick="FileList.deleteFile(${file.id})" title="${I18n.t("files.tooltip_delete")}">
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>
                </button>
              </div>
            </td>
      ...
      container.innerHTML = `<div class="shadcn-card p-10 text-center"><p class="text-error">${e.detail || I18n.t("files.load_error")}</p></div>`;
    }
  }
