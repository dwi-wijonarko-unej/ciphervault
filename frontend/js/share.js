const ShareUI = (() => {
  async function openShareModal(fileId, fileName) {
    UI.modal(
      "Share File",
      `
        <p class="text-secondary mb-4">Share <strong>"${fileName}"</strong> with another user.</p>
        <div class="flex items-center gap-2">
          <div class="flex-1">
            <label class="block text-xs font-medium text-secondary uppercase tracking-wider mb-1.5">Recipient Username</label>
            <input class="w-full px-3.5 py-2.5 bg-surface-input border border-border rounded-md text-sm text-primary placeholder-muted outline-none transition-all focus:border-[#2d6a4f] focus:ring-[3px] focus:ring-[rgba(45,106,79,0.1)]" id="share-recipient" placeholder="Enter username..." autocomplete="off">
          </div>
        ...
        <div class="mt-3">
          ...
          <input class="w-full px-3.5 py-2.5 bg-surface-input border border-border rounded-md text-sm text-primary placeholder-muted outline-none transition-all focus:border-[#2d6a4f] focus:ring-[3px] focus:ring-[rgba(45,106,79,0.1)]" id="share-expires" type="number" placeholder="Leave empty for no expiry" min="1" max="720">
        </div>
    ...
    setTimeout(() => {
      ...
      if (btn && input) {
        btn.addEventListener("click", async () => {
          ...
          if (!recipient) {
            result.innerHTML =
              '<p class="text-xs" style="color: var(--error);">Please enter a recipient username.</p>';
            return;
          }
          btn.disabled = true;
          btn.innerHTML =
            '<div class="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></div> Sharing...';
          try {
            ...
            result.innerHTML = `<p class="text-xs" style="color: var(--success);">✓ Shared with <strong>${res.recipient.username}</strong>. Token: <code class="bg-surface px-1.5 py-0.5 rounded text-[10px] font-mono">${res.access_token.substring(0, 16)}...</code></p>`;
            UI.toast(`File shared with ${res.recipient.username}`, "success");
          } catch (e) {
            result.innerHTML = `<p class="text-xs" style="color: var(--error);">✗ ${e.detail || "Share failed"}</p>`;
            btn.disabled = false;
            btn.innerHTML = "Share";
          }
        });
...
const SharedList = (() => {
  async function render(container) {
    ...
    try {
      ...
      if (res.items.length === 0) {
        container.innerHTML = `
          <div class="flex flex-col items-center justify-center py-16 text-center animate-[fadeIn_0.3s_ease]">
            ...
            <h3 class="text-lg font-bold font-heading">No shared files</h3>
      ...
      container.innerHTML = `<div class="bg-surface-card border border-border rounded-lg p-10 text-center"><p class="text-error">${e.detail || "Failed to load shared files"}</p></div>`;
    }
  }
