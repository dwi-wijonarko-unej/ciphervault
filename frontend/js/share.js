const ShareUI = (() => {
  async function openShareModal(fileId, fileName) {
    UI.modal(
      I18n.t("share.title"),
      `
        <p class="text-muted-foreground mb-4">${I18n.t("share.desc_prefix")} <strong>"${fileName}"</strong> ${I18n.t("share.desc_suffix")}</p>
        <div class="flex items-center gap-2">
          <div class="flex-1">
            <label class="block text-xs font-medium text-muted-foreground uppercase tracking-wider mb-1.5">${I18n.t("share.recipient")}</label>
            <input class="shadcn-input" id="share-recipient" placeholder="${I18n.t("share.recipient_placeholder")}" autocomplete="off">
          </div>
        ...
        <div class="mt-3">
          <label class="block text-xs font-medium text-muted-foreground uppercase tracking-wider mb-1.5">${I18n.t("share.expires")}</label>
          <input class="shadcn-input" id="share-expires" type="number" placeholder="${I18n.t("share.expires_placeholder")}" min="1" max="720">
        </div>
      ...
      if (btn && input) {
        btn.addEventListener("click", async () => {
          ...
          try {
            ...
            result.innerHTML = `<p class="text-xs" style="color: var(--success);">${I18n.t("share.success_prefix")} <strong>${res.recipient.username}</strong>. ${I18n.t("share.token_label")} <code class="bg-muted px-1.5 py-0.5 rounded text-[10px] font-mono">${res.access_token.substring(0, 16)}...</code></p>`;
...
const SharedList = (() => {
  async function render(container) {
    ...
    try {
      ...
      if (res.items.length === 0) {
        container.innerHTML = `
          <div class="flex flex-col items-center justify-center py-16 text-center animate-[fadeIn_0.3s_ease]">
            ...
            <p class="text-sm text-muted-foreground mt-2">${I18n.t("shared.empty_desc")}</p>
          </div>`;
        return;
      }
                ...
                <table class="w-full">
                  <thead>
                    <tr class="bg-muted border-b border-border">
                      ...
                      <th class="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">${I18n.t("shared.shared_by")}</th>
                      <th class="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider hidden sm:table-cell">${I18n.t("common.size")}</th>
      ...
      res.items.forEach((file) => {
        ...
        html += `
          <tr class="border-b border-border last:border-0 hover:bg-muted transition-colors duration-150 file-list-enter">
            <td class="px-4 py-3">
              <div class="flex items-center gap-2.5">
                ...
                <div class="min-w-0">
                  ...
                  <div class="text-xs text-muted-foreground">ID: ${file.id}</div>
                </div>
              </div>
            </td>
            <td class="px-4 py-3">
            ...
            <td class="px-4 py-3 text-sm text-muted-foreground whitespace-nowrap hidden sm:table-cell">${file.file_size_formatted || "—"}</td>
            <td class="px-4 py-3 text-sm text-muted-foreground whitespace-nowrap hidden md:table-cell">${new Date(file.created_at).toLocaleDateString("en-ID", { day: "numeric", month: "short", year: "numeric" })}</td>
            <td class="px-4 py-3 text-right">
              <button class="flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm text-muted-foreground hover:text-primary hover:bg-muted transition-all cursor-pointer bg-transparent border-none" onclick="Download.handle(${file.id}, '${file.filename_original}')">
      ...
      container.innerHTML = `<div class="shadcn-card p-10 text-center"><p class="text-error">${e.detail || I18n.t("activity.load_error")}</p></div>`;
    }
  }
