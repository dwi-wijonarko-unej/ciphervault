const Upload = (() => {
  function render(container) {
    container.innerHTML = `
      <div class="shadcn-card p-6 mb-6 shadow-sm">
        <div class="border-2 border-dashed rounded-lg p-10 text-center transition-all duration-200 cursor-pointer drop-zone" style="border-color: var(--input);" id="drop-zone">
          <svg class="w-12 h-12 mx-auto mb-3" style="color: var(--muted-foreground);" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
          <h3 class="text-base font-semibold mb-1">${I18n.t("upload.dropzone")}</h3>
          <p class="text-sm text-muted-foreground">${I18n.t("upload.browse")}</p>
  ...
  async function uploadFile(file) {
    ...
    item.className =
      "flex items-center gap-3 p-3 bg-muted rounded-lg animate-[fadeIn_0.3s_ease] mb-2";
    ...
    item.innerHTML = `
      ...
      <div class="flex-1 min-w-0">
        ...
        <div class="text-xs text-muted-foreground upload-status">${formatFileSize(file.size)} — ${I18n.t("upload.encrypting")}</div>
      </div>
      <div class="w-[120px] h-1.5 bg-muted rounded-full overflow-hidden">
