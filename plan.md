...
- Manajemen kunci sesi (*session key*) yang aman per file  
- Pengunggahan file aman (*secure upload*)  
- Pengunduhan file aman dengan verifikasi integritas (*secure download*)  
- Pembagian file aman melalui mekanisme *re-encryption key* (*secure sharing*)  
...
Fase 1     ██ FONDASI & AUTENTIKASI + CONFIG ██

                              │

Fase 2                        ██ HYBRID UPLOAD + SECURITY ANALYZER ██

                                             │

Fase 3                                          ██ DOWNLOAD + RSA SHARING ██

                                                              │

Fase 4                                                           ██ SYSTEM PAGE + POLISH ██

Milestone:

  **M1 (12 Juli):** Auth + Config + UHC(257) + AI Selector + RSA engine + Logistic Map teruji

  **M2 (19 Juli):** Hybrid Upload (UHC→AES→RSA wrap) + Security Analyzer + System config API

  **M3 (26 Juli):** Download + Sharing via RSA + File detail panel + Security score modal

  **M4 (2 Agustus):** System Info page + All 67+ tests pass + README siap
...
| Hari | Pekerjaan |
| :---- | :---- |
| Senin (6 Jul) | Setup: virtualenv, requirements.txt, .gitignore, struktur folder |
| Selasa (7 Jul) | Fase 0: uhc\_engine.py mod 257 + logistic\_map.py |
| Rabu (8 Jul) | Fase 0: aes\_engine.py + test\_crypto\_uhc.py + test\_crypto\_aes.py |
| Kamis (9 Jul) | Fase 1: config.py, database.py, models/user.py, schemas |
| Jumat (10 Jul) | Fase 1: utils, middleware, auth\_service, auth router, main.py |
| Sabtu (11 Jul) | Fase 1: frontend login.html, css, js (api, auth, ui) |
| Minggu (12 Jul) | Review M1: test auth + test crypto → all pass |

#### Minggu 2 (13-19 Juli) — Fase 2

| Hari | Pekerjaan |
| :---- | :---- |
| Senin (13 Jul) | key\_manager.py, metadata\_generator.py, integrity.py |
| Selasa (14 Jul) | models: stored\_file, file\_key, activity\_log. schemas/file.py |
| Rabu (15 Jul) | storage/base.py, local\_storage.py, storage/\_\_init\_\_.py |
| Kamis (16 Jul) | upload\_service.py + file\_service.py (get\_user\_files) |
| Jumat (17 Jul) | routers: upload.py, files.py + main.py update |
| Sabtu (18 Jul) | Frontend: index.html, components.css, app.js, upload.js, files.js |
| Minggu (19 Jul) | Review M2: test upload flow + test key manager → all pass |

#### Minggu 3 (20-26 Juli) — Fase 3

| Hari | Pekerjaan |
| :---- | :---- |
| Senin (20 Jul) | download\_service.py + file\_service.py (verify\_ownership) |
| Selasa (21 Jul) | routers/download.py + frontend download.js + files.js update |
| Rabu (22 Jul) | models/share.py, schemas/share.py, config update (server wrapping key) |
| Kamis (23 Jul) | share\_service.py + file\_service.py (get\_shared\_with\_me) |
| Jumat (24 Jul) | routers/share.py + files.py (shared) + main.py update |
| Sabtu (25 Jul) | Frontend: share.js + files.js update (share button, shared tab) |
| Minggu (26 Jul) | Review M3: test download + test share flow → all pass |

#### Minggu 4 (27 Juli-2 Agustus) — Fase 4

| Hari | Pekerjaan |
| :---- | :---- |
| Senin (27 Jul) | search\_service.py + file\_service.py (delete, search, verify) |
| Selasa (28 Jul) | routers/files.py: GET search, DELETE, POST verify |
| Rabu (29 Jul) | Frontend: search.js, animations.css, ui.js update |
| Kamis (30 Jul) | tests: conftest.py, test\_auth.py, test\_file\_management.py |
| Jumat (31 Jul) | tests: test\_security.py + final integration test run |
| Sabtu (1 Agu) | Bug fix, README.md, final review |
| Minggu (2 Agu) | **BUFFER + DELIVERY** |
...
*Dokumen ini berfungsi sebagai panduan tunggal (single source of truth) untuk pengembangan CipherVault. Setiap perubahan desain atau arsitektur harus didokumentasikan di sini.*  
